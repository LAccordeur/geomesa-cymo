 ...
 */
public class UberData implements CommonData {
    ...
    public SimpleFeatureType getSimpleFeatureType() {
        if (sft == null) {
            ...
            attributes.append("vehicleId:String,");
    ...
    public List<SimpleFeature> getTestData() {
        if (features == null) {
            ...
            URL input = getClass().getClassLoader().getResource("dataset/uber-raw-data-ordered-format.csv");
            if (input == null) {
                throw new RuntimeException("Couldn't load resource uber-raw-data.csv");
            }

            // date parser corresponding to the CSV format
            DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("MM/dd/yyyy HH:mm:ss", Locale.US);
            ...
            try (CSVParser parser = CSVParser.parse(input, StandardCharsets.UTF_8, CSVFormat.DEFAULT)) {
                for (CSVRecord record : parser) {
                    ...
                    builder.set("vehicleId", record.get(3));
                    ...
                    builder.set("dtg", Date.from(LocalDateTime.parse(record.get(0), dateFormat).toInstant(ZoneOffset.UTC)));
                    ...
                    features.add(builder.buildFeature(record.get(3)));
                }
            } catch (IOException e) {
                throw new RuntimeException("Error reading uber data:", e);
            }
