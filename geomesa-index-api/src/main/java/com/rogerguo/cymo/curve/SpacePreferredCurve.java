 ...
 * @Created by rogerguo
 */
public class SpacePreferredCurve implements SpaceFillingCurve {
    ...
    public long getCurveValue(int x, int y, int z) {
        ...
        int timeBitNum1 = (int) Math.ceil(Math.log(VirtualLayerConfiguration.TEMPORAL_PARTITION_A_LENGTH) / Math.log(2D));
        int timeBitNum2 = (int) Math.ceil(Math.log(VirtualLayerConfiguration.TEMPORAL_PARTITION_B_LENGTH) / Math.log(2D));
        int validTimeBitNum = timeBitNum1 > timeBitNum2 ? timeBitNum1 : timeBitNum2;
        long spacePreferred = (spatialPart << validTimeBitNum) | z;

        return spacePreferred;
    }
    ...
    public static void main(String[] args) {
        SpacePreferredCurve timePreferredCurve = new SpacePreferredCurve();
